--[[
AI Exposure Assist — Prepare Selected Folder Job

Collects the current Lightroom selection, asks Python to snapshot the preview
cache and extract all previews once, then stops. It does not call an AI and it
does not write XMP.
]]
...
function RunExposureAssist.run()
    if RunExposureAssist.isRunning then
        LrDialogs.message("AI Exposure Assist", "A prepare job is already running.", "warning")
        return
    end
    ...
    local success, err = LrTasks.pcall(function()
        ...
        if #photos == 0 then
            LrDialogs.message(
                "AI Exposure Assist",
                "No photos are selected. Open the intended Lightroom folder, select all intended photos, and run Prepare again.",
                "info"
            )
            return
        end
        ...
        local progress = LrProgressScope({
            ...
            caption = "Preparing previews for " .. tostring(#photos) .. " photos..."
        })
        ...
        LrDialogs.message(
            "AI Exposure Assist — Job Prepared",
            "Prepared " .. tostring(result.total_found or 0) .. " previews from "
                .. tostring(result.total_selected or #photos) .. " selected photos.\n\n"
                .. "Give this folder to any vision-capable AI app:\n"
                .. tostring(result.job_dir) .. "\n\n"
                .. "The AI must read AI_TASK.md and save decisions in:\n"
                .. tostring(result.decision_directory) .. "\n\n"
                .. "After the decisions are complete, run ‘AI Exposure Assist — Apply Prepared Job’.",
            "info"
        )
    end)
