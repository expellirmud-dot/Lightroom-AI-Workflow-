...
| VLD-151 | 2026-09-06 | Canonical diagnostic remediation: focused diagnostics 14/14, full pytest exit 0 with 2 expected skips, plus read-only rerun of captured Owner request `diagnostic-1788691404` against real cache | PASS for semantic repair: metadata sync = `NOT_REQUIRED_CATALOG_AUTHORITATIVE`; preview-cache DB = PASS; legacy XMP = PASS; canonical Standard Preview check finds 359/393 ready and 34 `PREVIEW_TIER_NOT_READY`; old RootPixels missing condition no longer falsely blocks. This is integrated/read-only evidence, not post-fix Lightroom-host proof. |
