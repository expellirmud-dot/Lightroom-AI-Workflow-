...
Prepare all selected Lightroom previews once, let any file-capable vision AI analyze the exported job folder, save structured decisions, and then apply only validated `crs:Exposure2012` changes to the original XMP sidecars.
...
```text
Lightroom folder
→ select all intended photos
→ Prepare Selected Folder
...
The Lightroom command **AI Exposure Assist — Prepare Selected Folder**:

1. Reads the current Lightroom selection.
2. Writes `selection.json` with `image_id`, RAW path, and Lightroom UUID.
3. Snapshots `previews.db` and `root-pixels.db` read-only.
4. Extracts one current-rendered JPEG preview per selected image where available.
5. Writes a durable job folder.
6. Stops without calling AI and without touching XMP.
...
A folder job must contain photos from exactly one source folder. Large folders may be selected in bounded groups, but each group is prepared only once.

## Phase 2 — External AI review

The AI reads `AI_TASK.md`, `manifest.json`, and every FOUND preview. It must use all four project skills:
...
The AI never edits RAW, XMP, Lightroom catalog, preview cache, manifest, or preview files.
...
Validation rejects the whole analysis batch before apply when decision files are missing, unknown, duplicated, malformed, outside the job directory, or identity-mismatched. Validated decisions are written to `ai-decisions.json`, `analysis-records.json`, and `analysis-evidence.json`.
...
`apply-evidence.json` is checkpointed after every image. Settled images are not processed twice. Every selected image receives one terminal record, including images whose previews could not be extracted.
