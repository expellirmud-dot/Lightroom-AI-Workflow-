...
| CAP-043 | Diagnostic-first current-folder report | INTEGRATED | WO-030, WO-031, WO-041 | canonical diagnostic now checks Standard Preview `>=preview_size`, treats RootPixels/XMP/metadata-sync as legacy/advisory, focused/full tests green, and exact captured 393-image request rechecked read-only against real cache | Owner rerun in Lightroom; no promotion until host result is observed |
